  ...
  return (
    <section id="how-it-works" className="relative overflow-hidden py-16 sm:py-24 lg:py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
        ...
        <div className="relative mt-10 sm:mt-16 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4">
          ...
          {steps.map(({ icon: Icon, step, title, text }, i) => (
            <motion.div
              ...
              className="relative rounded-2xl glass p-4 text-center sm:p-6"
            >
              <div className="mx-auto flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl gradient-primary text-white shadow-lg shadow-primary-500/30">
                <Icon className="h-5 w-5 sm:h-6 sm:w-6" />
              </div>
              <p className="mt-3 sm:mt-4 text-xs font-bold uppercase tracking-widest text-primary-600">
                Step {step}
              </p>
              <h3 className="mt-1.5 sm:mt-2 font-heading text-base sm:text-lg font-bold">{title}</h3>
              <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-slate-600 dark:text-slate-300">{text}</p>
            </motion.div>
          ))}
        </div>
      </div>
