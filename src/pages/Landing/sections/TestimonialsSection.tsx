        ...
        <div className="mt-10 sm:mt-14 grid gap-4 sm:gap-6 md:grid-cols-3">
          {testimonials.map((t, i) => (
            <motion.blockquote
              ...
              className="relative rounded-2xl glass p-4 sm:p-6"
            >
              <Quote className="absolute right-4 top-4 sm:right-5 sm:top-5 h-7 w-7 sm:h-8 sm:w-8 text-primary-200 dark:text-primary-800" />
              <div className="flex gap-0.5 text-amber-400">
                {Array.from({ length: 5 }).map((_, s) => (
                  <Star key={s} className="h-3.5 w-3.5 sm:h-4 sm:w-4 fill-current" />
                ))}
              </div>
              <p className="mt-3 sm:mt-4 text-xs sm:text-sm leading-relaxed text-slate-700 dark:text-slate-200">
                &ldquo;{t.quote}&rdquo;
              </p>
              <footer className="mt-4 sm:mt-6">
                <p className="font-heading text-xs sm:text-sm font-bold">{t.name}</p>
