...
export default function CreateComplaintPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div>
        <h1 className="font-display text-xl sm:text-2xl font-extrabold">File a Complaint</h1>
      ...
      <ol className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-2">
        {STEPS.map((s, i) => {
          const Icon = s.icon
          ...
          return (
            <li key={s.id} className="flex flex-1 items-center gap-1.5 sm:gap-2 min-w-0 sm:min-w-[4.5rem]">
              <button
                type="button"
                onClick={() => tryAdvanceStep(s.id)}
                disabled={isFutureBlocked && !completedSteps.has(1)}
                className={cn(
                  'flex h-8 w-8 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold transition',
                  active || done
